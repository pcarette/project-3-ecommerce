...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    if ('_id' in cartState) {
      axios
        .get(`${API_URL}/cart/${cartState._id}`, {
          headers: {
  ...
  const updateThroughDownloadedCart = () => {
    ...
    const storedToken = localStorage.getItem('authToken')
    const filteredCart = downloadedCart.map((prod) => {
      return {
  ...
  return (
    <div>
      ...
      <ul>
        {downloadedCart &&
          downloadedCart.length > 0 &&
          downloadedCart.map((product, i) => {
            return (
              <Link to={`/product/${product._id}`}>
                <li key={`cart-${product._id}-${i}`}>
                  ...
                  <div className='toRemove' onClick={() => removeItemInDownloadedCart(product.productId._id)}>
                    Remove
                  </div>
                </li>
              </Link>
