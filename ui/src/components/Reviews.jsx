...
export default function Reviews() {
  ...
  const updateReviews = (page) => {
    setIsLoading(true)
    axios
      .get(`${API_URL}/review/${id}?page=${page}`)
      .then((res) => {
      ...
      {isLoaded && (
        <div>
          <div className='reviewMenu'>
            <button className='changeReviewsPage' onClick={() => changePage(-1)}>
              -
            </button>
            <p>
              {reviewsState.pagesInfo.page}/{reviewsState.pagesInfo.totalPageNumber}
            </p>
            <button className='changeReviewsPage' onClick={() => changePage(1)}>
              +
            </button>
