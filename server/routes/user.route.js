...
router.put('/', authentication, async (req, res, next) => {
  const availableKeys = ['email', 'password', 'firstName', 'lastName', 'address', 'isMailValidated']
  const wrongFields = Object.keys(req.body).filter((key) => !availableKeys.includes(key))
